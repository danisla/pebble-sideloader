...
Pebble = function(ip, port) {
  ...
  this.install_app = function(url) {
    ...
    output_feedback("Opening file");
    request.addEventListener("error",
                             function(event){
                                output_feedback("Can not access file. Premissions on file server will not allow. See if file provider can provide a public link.");
                             }
    , false);
  ...
  var handle_socket_error = function(e) {
    ...
    output_feedback("Socket error: " + e);
    self.trigger('error', e);
  };

  var handle_socket_open = function(e) {
    console.log("Socket open");
    output_feedback("Socket open");
    mHasConnected = true;
    self.trigger('open');
  };
  ...
  var handle_socket_close = function(e) {
    ...
    output_feedback("Socket closed.");
    if(!e.wasClean) {
      console.log("Close was unexpected.");
      output_feedback("Close was unexpected.");
      if(!mHasConnected) {
        output_feedback("error", "Connection to the phone failed. Check the IP and that developer mode is active.");
        ...
        output_feedback("error", "Connection to the phone was interrupted.");
      }
    } else {
      console.log("phone disconnect clean");
      output_feedback("Phone disconnect clean");
    }
    self.trigger('close');
  };
  ...
  var handle_socket_message = function(e) {
    ...
    if(origin == 5) {
      ...
      output_feedback("Received status update: ", result[0]);
...
1: 'tintin-black',
  ...
  var handle_failure = function() {
    ...
    output_feedback("The attached watch has no concept of colour! So, red.");
...
var send_message = function(endpoint, message) {
  ...
  if(mSocket.readyState != WebSocket.OPEN) {
    ...
    output_feedback("Cannot send on non-open socket.");
  }
  output_feedback("Sending data");
  mSocket.send(data);
};
